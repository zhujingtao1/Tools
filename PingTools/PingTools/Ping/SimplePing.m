...
- (void)start {
    ...
    addr.sin_port = 8484; // 如果不需要端口，则设置为 0

    int result = inet_pton(AF_INET, [self.hostName UTF8String], &addr.sin_addr);
    if (result == 1) {
        self.hostAddress = [NSData dataWithBytes:&addr length:sizeof(addr)];
        [self startWithHostAddress];
    } else {
        NSLog(@"转换IP失败");
        [self didFailWithHostName];
    }
}
